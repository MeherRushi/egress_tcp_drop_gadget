...
int egress_tcp_drop(struct __sk_buff *skb){
	...
	if(egress == false)								// If the egress flag is false, let the packets pass through
		return TC_OK;
		
	bpf_printk("Hello Working egress");
...
int ingress_tcp_drop(struct __sk_buff *skb){
	
	if(ingress == false)							// If the ingress flag is false, let the packets pass through
		return TC_OK;
		
	bpf_printk("Hello Working ingress");
